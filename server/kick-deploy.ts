  ...
  defineCliPlugin({
    ...
    register(program, ctx) {
      ...
      async function bundle(): Promise<string> {
        ...
        const { devtoolsFlagPlugin, devtoolsStripPlugin } =
          await import("@forinda/kickjs-vite");
...
`,
          ...
          ctx.log(
            `wrote ${relative(process.cwd(), resolve(functions, "api.mjs"))}`,
          );
        });
      ...
      program
        ...
        .description(
          "Bundle the API and write .vercel/output (Build Output API v3)",
        )
        .action(async () => {
          const server = await bundle();
